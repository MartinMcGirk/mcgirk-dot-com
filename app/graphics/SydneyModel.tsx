...
export function SydneyModel(props: any) {
    ...
    useFrame((state, delta, frame) => {
        ...
        const parallaxY = mouseCoords[1] * 0.5;

        groupRef.current.position.x += (parallaxX - groupRef.current.position.x) * 5 * delta;
        groupRef.current.position.y += (parallaxY - groupRef.current.position.y) * 5 * delta;
    });
